...
handle_node_version() {
    ...
    if [ -n "$NODE_LINE" ] && echo "$NODE_LINE" | grep -q '\^'; then
        ...
        if [ -n "$NODE_VERSION" ] && [ "$NODE_VERSION" -gt 20 ]; then
            if [ "$NODE_VERSION" -gt 22 ]; then
                # TODO: NODE_VERSION test should check for 22 and not 24 but checking for 22 temporarily so that script is able to proceed
                if [ "$NODE_VERSION" -gt 24 ]; then
